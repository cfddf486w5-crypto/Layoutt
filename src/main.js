...
function cellFeet(){
  return +(cellSizeInch / 12).toFixed(4);
}

function cellSurfaceFt2(){
  return +((cellSizeInch / 12) * (cellSizeInch / 12)).toFixed(2);
}

const STRUCTURAL_TOOLS = new Set([
  'wall_solid_int', 'wall_solid_ext', 'wall_light_partition', 'wall_mesh_fence', 'pillar_column',
  'guard_rail', 'curb_barrier', 'window_opening', 'floor_hole'
]);

const DOOR_TOOLS = new Set([
  'door_single', 'door_double', 'door_swing_left', 'door_swing_right', 'door_sliding',
  'door_rollup', 'door_fire', 'door_security_badge', 'gate_cage', 'curtain_strip', 'dock_door', 'turnstile'
]);

const PLAN_TOOLS = new Set([
  'aisle_walkway', 'aisle_forklift', 'oneway_n', 'oneway_e', 'oneway_s', 'oneway_w', 'crosswalk', 'stop_sign', 'no_entry', 'hazard_zone',
  'rack_pallet', 'rack_cantilever', 'shelving', 'mezzanine', 'floor_stack', 'empty_pallet_area', 'tote_area', 'cage_storage',
  'bin_p1', 'bin_p2', 'bin_p3', 'bin_p4', 'bin_p5', 'bin_p6', 'bin_p7', 'bin_free', 'bin_blocked', 'bin_reserved', 'bin_unknown_type', 'reception_staging',
  'zone_receiving', 'zone_qc', 'zone_putaway', 'zone_picking', 'zone_consolidation', 'zone_packing', 'zone_shipping', 'zone_crossdock',
  'zone_returns', 'zone_scrap', 'zone_rebox', 'zone_exceptions',
  'dock_platform', 'dock_leveler', 'ramp', 'conveyor_fixed', 'conveyor_extendable', 'lift_table', 'wrap_station', 'scale_station', 'printer_station', 'battery_charging',
  'office', 'customer_counter', 'toilet', 'cafeteria', 'locker_room', 'electrical_room', 'it_closet',
  'label_text', 'marker_entry', 'marker_exit', 'marker_measure'
]);

const TYPE_COLORS = {
  wall_solid_int: '#6b7280',
  wall_solid_ext: '#4b5563',
  wall_light_partition: '#9ca3af',
  wall_mesh_fence: '#64748b',
  pillar_column: '#52525b',
  guard_rail: '#22d3ee',
  curb_barrier: '#64748b',
  window_opening: '#93c5fd',
  floor_hole: '#1f2937',
  door_single: '#12b76a',
  door_double: '#16a34a',
  door_swing_left: '#22c55e',
  door_swing_right: '#22c55e',
  door_sliding: '#10b981',
  door_rollup: '#14b8a6',
  door_fire: '#ef4444',
  door_security_badge: '#0ea5e9',
  gate_cage: '#84cc16',
  curtain_strip: '#f59e0b',
  dock_door: '#06b6d4',
  turnstile: '#22d3ee',
  aisle_walkway: '#10b981',
  aisle_forklift: '#0ea5e9',
  oneway_n: '#3b82f6',
  oneway_e: '#3b82f6',
  oneway_s: '#3b82f6',
  oneway_w: '#3b82f6',
  ...
  hazard_zone: '#ef4444',
  rack_pallet: '#fb923c',
  rack_cantilever: '#fdba74',
  shelving: '#facc15',
  mezzanine: '#94a3b8',
  floor_stack: '#475569',
  empty_pallet_area: '#a16207',
  tote_area: '#65a30d',
  cage_storage: '#64748b',
  battery_charging: '#14b8a6',
  bin_p1: '#164e63',
  bin_p2: '#0f766e',
  bin_p3: '#15803d',
  bin_p4: '#65a30d',
  ...
  bin_blocked: '#7f1d1d',
  bin_reserved: '#4338ca',
  bin_unknown_type: '#64748b',
  reception_staging: '#818cf8',
  zone_receiving: '#60a5fa',
  zone_qc: '#a78bfa',
  zone_putaway: '#c084fc',
  zone_picking: '#f472b6',
  zone_consolidation: '#fb7185',
  zone_packing: '#fb923c',
  zone_shipping: '#f59e0b',
  zone_crossdock: '#facc15',
  zone_returns: '#4ade80',
  zone_scrap: '#ef4444',
  zone_rebox: '#f97316',
  zone_exceptions: '#06b6d4',
  dock_platform: '#0284c7',
  dock_leveler: '#0ea5e9',
  ramp: '#0f766e',
  conveyor_fixed: '#2563eb',
  conveyor_extendable: '#1d4ed8',
  lift_table: '#8b5cf6',
  wrap_station: '#7c3aed',
  scale_station: '#8b5cf6',
  printer_station: '#6366f1',
  office: '#c084fc',
  customer_counter: '#e879f9',
  toilet: '#38bdf8',
  cafeteria: '#f472b6',
  locker_room: '#60a5fa',
  electrical_room: '#ef4444',
  it_closet: '#6366f1',
  label_text: '#e2e8f0',
  marker_entry: '#14b8a6',
  marker_exit: '#16a34a',
  marker_measure: '#38bdf8'
};

const CUSTOM_TOOL_BUTTONS = [
  ['wall_solid_int', 'Mur plein (intérieur)'], ['wall_solid_ext', 'Mur plein (extérieur)'], ['wall_light_partition', 'Cloison légère'], ['wall_mesh_fence', 'Mur grillagé'],
  ['pillar_column', 'Colonne / pilier'], ['guard_rail', 'Garde-corps'], ['curb_barrier', 'Bordure / butée'], ['window_opening', 'Fenêtre'], ['floor_hole', 'Ouverture au sol (trémie)'],
  ['door_single', 'Porte simple'], ['door_double', 'Porte double'], ['door_swing_left', 'Porte battante (charnière G)'], ['door_swing_right', 'Porte battante (charnière D)'],
  ['door_sliding', 'Porte coulissante'], ['door_rollup', 'Porte de garage (roll-up)'], ['door_fire', 'Porte coupe-feu'], ['door_security_badge', 'Porte sécurisée (badge)'],
  ['gate_cage', 'Portail de cage'], ['curtain_strip', 'Rideau industriel (lanières)'], ['dock_door', 'Porte de quai'], ['turnstile', 'Tourniquet'],
  ['aisle_walkway', 'Allée piétonne'], ['aisle_forklift', 'Allée chariot'], ['oneway_n', 'Sens unique ↑'], ['oneway_e', 'Sens unique →'],
  ['oneway_s', 'Sens unique ↓'], ['oneway_w', 'Sens unique ←'], ['crosswalk', 'Passage piéton'], ['stop_sign', 'Stop'], ['no_entry', 'Accès interdit'], ['hazard_zone', 'Zone danger'],
  ['rack_pallet', 'Rack palettes (standard)'], ['rack_cantilever', 'Rack cantilever'], ['shelving', 'Étagères / shelving'], ['mezzanine', 'Mezzanine (zone)'], ['floor_stack', 'Stock au sol'],
  ['empty_pallet_area', 'Zone palettes vides'], ['tote_area', 'Zone bacs/totes'], ['cage_storage', 'Cage grillagée (valeur)'], ['battery_charging', 'Charge batteries chariots'],
  ['bin_p1', 'Bin P1'], ['bin_p2', 'Bin P2'], ['bin_p3', 'Bin P3'], ['bin_p4', 'Bin P4'], ['bin_p5', 'Bin P5'], ['bin_p6', 'Bin P6'], ['bin_p7', 'Bin P7'],
  ['bin_free', 'Bin libre (visuel)'], ['bin_blocked', 'Bin bloqué (maintenance)'], ['bin_reserved', 'Bin réservé'], ['bin_unknown_type', 'TYPE INCONNU'], ['reception_staging', 'RECEPTION_STAGING (virtuel)'],
  ['zone_receiving', 'Zone Réception'], ['zone_qc', 'Contrôle qualité (QC)'], ['zone_putaway', 'Mise en stock (Putaway)'], ['zone_picking', 'Picking'],
  ['zone_consolidation', 'Consolidation'], ['zone_packing', 'Packing'], ['zone_shipping', 'Expédition (Shipping)'], ['zone_crossdock', 'Cross-dock'], ['zone_returns', 'Retours'],
  ['zone_scrap', 'Scrap / Rejet'], ['zone_rebox', 'Rebox / Reconditionnement'], ['zone_exceptions', 'À valider / Exceptions'],
  ['dock_platform', 'Quai (plateforme)'], ['dock_leveler', 'Niveleur de quai'], ['ramp', 'Rampe'], ['conveyor_fixed', 'Convoyeur fixe'], ['conveyor_extendable', 'Convoyeur extensible'],
  ['lift_table', 'Table élévatrice'], ['wrap_station', 'Station film étirable'], ['scale_station', 'Balance / pesée'], ['printer_station', 'Poste imprimante étiquettes'],
  ['office', 'Bureau'], ['customer_counter', 'Comptoir service client'], ['toilet', 'Toilette'], ['cafeteria', 'Cafétéria'], ['locker_room', 'Vestiaire'],
  ['electrical_room', 'Local électrique'], ['it_closet', 'Local IT / réseau'], ['label_text', 'Texte / étiquette'], ['marker_entry', 'Point d’entrée'], ['marker_exit', 'Point de sortie'], ['marker_measure', 'Repère mesure / échelle']
];
...
function selectedFraction(){
  const value = Number.parseFloat(cellFractionEl?.value || '1');
  if([1, 0.5, 0.3333333333, 0.25].some((v) => Math.abs(v - value) < 0.0001)) return value;
  return 1;
}

function fractionLabel(value){
  const key = Object.keys(FRACTION_LABELS).find((k) => Math.abs(Number(k) - Number(value)) < 0.0001);
  return key ? FRACTION_LABELS[key] : 'entière';
}
...
function ensureDynamicTypeStyles(){
  ...
  const textColors = new Set(['crosswalk', 'bin_free', 'window_opening', 'label_text']);
...
function hydrateCell(src){
  ...
  return {
    ...
    zone: src.zone || '',
    sizeFraction: Number(src.sizeFraction) > 0 ? Number(src.sizeFraction) : 1
  };
}
...
function updateCoords(row, col){
  ...
  if(d.sizeFraction && d.sizeFraction < 1) parts.push('surface=' + fractionLabel(d.sizeFraction));
...
function handleBinRangeClick(row, col){
  ...
  if(binRangeState.phase === 'awaitSecond'){
    ...
    if(dist >= maxBinCells){ toast('Max ' + maxBinCells + ' cases.'); return; }
    ...
    for(const pos of cells){
      ...
      d.type = 'bin'; d.binId=null; d.depth=null; d.isHead=false; d.direction=null; d.label=''; d.sizeFraction=1;
...
function updateStats(){
  ...
  if(kpiSurfaceEl) kpiSurfaceEl.textContent = `${Math.round(totalCells * cellSurfaceFt2())} pi²`;
...
function bwInit(){
  ...
  bw.cellSizeEl = document.getElementById('bwCellSize');
  bw.maxCellsEl = document.getElementById('bwMaxCells');
  ...
  bw.cellSizeEl?.addEventListener('input', () => {
    const value = Number.parseInt(bw.cellSizeEl.value || '', 10);
    if(Number.isFinite(value) && value >= 12 && value <= 200) cellSizeInch = value;
    if(bw.cellSizeEl) bw.cellSizeEl.value = String(cellSizeInch);
    bwRefreshComputed();
    updateStats();
  });
  bw.maxCellsEl?.addEventListener('input', () => {
    const value = Number.parseInt(bw.maxCellsEl.value || '', 10);
    if(Number.isFinite(value) && value >= 2 && value <= Math.max(ROWS, COLS)) maxBinCells = value;
    if(bw.maxCellsEl) bw.maxCellsEl.value = String(maxBinCells);
    if(binRangeState.phase === 'awaitSecond' && binRangeState.start) markAvailableFromStart(binRangeState.start);
    bwRefreshComputed();
  });
...
function bwShow(step){
  ...
  bw.subEl.textContent = `Max ${maxBinCells} cases. Chaque case = ${cellSizeInch}×${cellSizeInch} pouces (${(cellSizeInch/12).toFixed(2)}×${(cellSizeInch/12).toFixed(2)} pieds).`;
  if(step===1){ bw.el.style.display='none'; bw.stepEl.textContent='Étape 1/4'; bw.msgEl.textContent='Choisis la 1ère case (départ).'; bw.formEl.style.display='none'; bw.validateBtn.disabled=true; }
  if(step===2){ bw.el.style.display='none'; bw.stepEl.textContent='Étape 2/4'; bw.msgEl.textContent='Choisis la 2e case (même ligne/colonne).'; bw.formEl.style.display='none'; bw.validateBtn.disabled=true; }
  if(step===3){ bw.el.style.display='none'; bw.stepEl.textContent='Étape 3/4'; bw.msgEl.textContent='Clique une extrémité pour définir la TÊTE.'; bw.formEl.style.display='none'; bw.validateBtn.disabled=true; }
  if(step===4){ bw.el.style.display='block'; bw.stepEl.textContent='Étape 4/4'; bw.msgEl.textContent='Confirme la création: Zone + Allée + Emplacement (et suffixe).'; bw.formEl.style.display='block'; if(bw.typeSearchEl) bw.typeSearchEl.value=''; filterBinModelSelect(''); bwRefreshComputed(); }
}
...
function bwRefreshComputed(){
  if(bw.subEl) bw.subEl.textContent = `Max ${maxBinCells} cases. Chaque case = ${cellSizeInch}×${cellSizeInch} pouces (${(cellSizeInch/12).toFixed(2)}×${(cellSizeInch/12).toFixed(2)} pieds).`;
  ...
  const ft=(count*cellFeet()).toFixed(2);
  ...
  bw.computedEl.textContent = `Code: ${info.ok?info.code:'(incomplet)'} • Cases: ${count} (${cellSizeInch}" x ${cellSizeInch}", ≈ ${ft} pieds) • Type: ${t} ${pr?('('+pr+')'):''}`;
  if(bw.modelInfoEl){
